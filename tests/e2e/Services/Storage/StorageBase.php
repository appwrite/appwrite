...
trait StorageBase
{
    ...
    public function testCreateBucketFileSnappyCompression(): array
    {
        $bucket = $this->client->call(Client::METHOD_POST, '/storage/buckets', [
            'content-type' => 'application/json',
            'x-appwrite-project' => $this->getProject()['$id'],
        ...
        $this->assertEquals(201, $bucket['headers']['status-code']);
        $this->assertNotEmpty($bucket['body']['$id']);
        $this->assertEquals('snappy', $bucket['body']['compression']);
    ...
    /**
     * @depends testCreateBucketFileSnappyCompression
     */
    public function testFilePreviewSnappyCompression(array $data): array
    {
        $bucketId = $data['bucketId'];
