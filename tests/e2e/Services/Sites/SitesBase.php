...
trait SitesBase
{
    ...
    protected function waitDeploymentReady(string $siteId, string $deploymentId): void
    {
        $this->assertEventually(function () use ($siteId, $deploymentId) {
            ...
            $code = Console::execute("docker inspect orchestrator --format='{{.State.ExitCode}}'", '', $this->stdout, $this->stderr);
            if ($code === 0 && \trim($this->stdout) !== '' && \trim($this->stdout) !== '0') {
                $msg = 'Orchestrator has a problem: ' . $this->stderr . ' (' . $this->stdout . '), current status: ';

                Console::execute("docker logs orchestrator", '', $this->stdout, $this->stderr);
