...
final class VCSGitHubConsoleClientTest extends Scope
{
    ...
    public function testPushMatchingSkipPatternCreatesNoDeployment(): void
    {
        ...
        $payload = [
            ...
            'head_commit' => [
                ...
                'message' => 'Update docs ' . VCS_DEPLOYMENT_SKIP_PATTERNS[0],
                'url' => $commit['commitUrl'],
            ],
            'commits' => [[
                'id' => $commit['commitHash'],
        ...
        $before = $this->listDeploymentIdsHelper($data['functionId']);

        $event = $this->client->call(Client::METHOD_POST, '/vcs/github/events', $headers, $payload);

        // The event is handled inside the request, so a 200 means it is fully processed.
        $this->assertEquals(200, $event['headers']['status-code']);
        $this->assertEqualsCanonicalizing($before, $this->listDeploymentIdsHelper($data['functionId']));
    }
    ...
    private function listDeploymentIdsHelper(string $functionId): array
    {
        ...
        return \array_column($deployments['body']['deployments'], '$id');
    }
