...
class FunctionsCustomClientTest extends Scope
{
    ...
    public function testCreateExecutionMaxLogs():array
    {
        $projectId = $this->getProject()['$id'];
        $apikey = $this->getProject()['apiKey'];
        ...
        $tag = $this->client->call(Client::METHOD_POST, '/functions/'.$function['body']['$id'].'/tags', [
            ...
            'command' => 'php index.php',
            'code' => new CURLFile(realpath(__DIR__ . '/../../../resources/functions/over-max-logs.tar.gz'), 'application/x-gzip', 'over-max-logs-fx.tar.gz'),
        ]);
        ...
        $this->assertCount(1, $executions['body']['executions']);
        $this->assertEquals('completed', $executions['body']['executions'][0]['status']);
        // $this->assertStringContainsString('Output has been truncated.', $executions['body']['executions'][0]['stderr']);
        $this->assertEquals(65535, mb_strlen($executions['body']['executions'][0]['stdout']));
