...
final class RealtimeCustomClientQueryTestWithMessage extends Scope
{
    ...
    public function testConsoleTailServerSideFilter(): void
    {
        ...
        // create ΓÇö must be filtered out (never reaches the token bucket).
        ...
        // delete ΓÇö must pass the filter.
    ...
    public function testConsoleTailUnauthorizedProjectIsRejected(): void
    {
        ...
        // A project the console user's team does not own (does not exist) ΓåÆ policy violation.
        $client->send(\json_encode([
            'type' => 'subscribe',
        ...
        // socket may already be dead ΓÇö both are valid rejections. What must NOT happen
        ...
        // close() can itself throw on an already-closed socket ΓÇö best effort.
        try {
            $client->close();
        } catch (ConnectionException $e) {
    ...
    public function testConsoleTailSeesEventsNotReadableByTheConsoleUser(): void
    {
        ...
        // Read restricted to a user that is NOT the console user ΓåÆ the console viewer
