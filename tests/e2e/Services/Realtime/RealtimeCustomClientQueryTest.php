...
class RealtimeCustomClientQueryTest extends Scope
{
    ...
    public function testExecutionChannelWithQuery()
    {
        $user = $this->getUser();
        $session = $user['session'] ?? '';
        $projectId = $this->getProject()['$id'];
        ...
        $this->assertEventually(function () use ($function, $deploymentId, $projectId) {
        ...
        // Subscribe with query for execution with response (not null)
        $client = $this->getWebsocket(['executions'], [
            'origin' => 'http://localhost',
            'cookie' => 'a_session_' . $projectId . '=' . $session,
        ], null, [
            Query::isNotNull('response')->toString(),
        ]);

        $response = json_decode($client->receive(), true);
        $this->assertEquals('connected', $response['type']);

        // Execute function - should receive event when execution completes with response
        ...
        $event = json_decode($client->receive(), true);
        if ($event['type'] === 'event' && isset($event['data']['payload']['response'])) {
            $this->assertEquals('event', $event['type']);
            $this->assertNotNull($event['data']['payload']['response']);
        }
        ...
        $targetTeamId = ID::unique();

        // Subscribe with query for specific team ID
        $client = $this->getWebsocket(['teams'], [
            'origin' => 'http://localhost',
            'cookie' => 'a_session_' . $projectId . '=' . $session,
        ], null, [
            Query::equal('$id', [$targetTeamId])->toString(),
        ]);

        $response = json_decode($client->receive(), true);
        $this->assertEquals('connected', $response['type']);

        // Create team with matching ID - should receive event
        ...
        $event = json_decode($client->receive(), true);
        $this->assertEquals('event', $event['type']);
        $this->assertEquals($targetTeamId, $event['data']['payload']['$id']);

        // Create team with different ID - should NOT receive event
        $otherTeamId = ID::unique();
