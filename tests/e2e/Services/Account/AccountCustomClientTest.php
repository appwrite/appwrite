...
class AccountCustomClientTest extends Scope
{
    ...
    public function testDeleteOtherAccountSessions(): void
    {
        $data = $this->setupAccountWithVerifiedEmail();
        $email = $data['email'];
        $password = $data['password'];
        ...
        $session2 = $response['cookies']['a_session_' . $this->getProject()['$id']];
        ...
        $this->assertEquals(201, $response['headers']['status-code']);
        $session3Id = $response['body']['$id'];
        $session3 = $response['cookies']['a_session_' . $this->getProject()['$id']];

        $response = $this->client->call(Client::METHOD_GET, '/account/sessions', array_merge([
            'origin' => 'http://localhost',
            'content-type' => 'application/json',
            'x-appwrite-project' => $this->getProject()['$id'],
            'cookie' => 'a_session_' . $this->getProject()['$id'] . '=' . $session3,
        ]));

        $this->assertEquals(200, $response['headers']['status-code']);
        $this->assertEquals(3, $response['body']['total']);

        $response = $this->client->call(Client::METHOD_DELETE, '/account/sessions/others', array_merge([
            'origin' => 'http://localhost',
            'content-type' => 'application/json',
            'x-appwrite-project' => $this->getProject()['$id'],
            'cookie' => 'a_session_' . $this->getProject()['$id'] . '=' . $session3,
        ...
        $this->assertEquals(200, $response['headers']['status-code']);
        $this->assertEquals(1, $response['body']['total']);
        $this->assertEquals($session3Id, $response['body']['sessions'][0]['$id']);
        $this->assertEquals(true, $response['body']['sessions'][0]['current']);
        ...
        $response = $this->client->call(Client::METHOD_GET, '/account', array_merge([
            ...
            'content-type' => 'application/json',
            'x-appwrite-project' => $this->getProject()['$id'],
            'cookie' => 'a_session_' . $this->getProject()['$id'] . '=' . $session1,
        ]));
