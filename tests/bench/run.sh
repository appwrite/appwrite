...
# A handler that does nothing at all. Every cell above has a handler expensive
# enough to hide the broker behind it -- which is the point when the question is
# which axis to scale, and useless when the question is what the broker itself
