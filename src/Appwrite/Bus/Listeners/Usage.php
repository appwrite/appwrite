    ...
    private function handleRequestCompleted(RequestCompleted $event, Publisher $publisher): void
    {
        ...
        if ($deployment->getAttribute('resourceType') === 'sites') {
            ...
            $queueForStatsUsage
                ...
                ->addMetric(str_replace('{siteInternalId}', $siteInternalId, METRIC_SITES_ID_INBOUND), $event->request->getSize() + $fileSize)
                ->addMetric(str_replace('{siteInternalId}', $siteInternalId, METRIC_SITES_ID_OUTBOUND), $event->response->getSize())
                ->trigger();
        } else {
            $queueForStatsUsage
                ->setProject($project)
