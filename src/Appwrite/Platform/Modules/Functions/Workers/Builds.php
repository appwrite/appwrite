...
class Builds extends Action
{
    ...
    protected function buildDeployment(
        ...
        if ($isVcsEnabled) {
            ...
            $provider = $vcs->getProviderForInstallation($installation);
        ...
        try {
            if (! $isVcsEnabled) {
                ...
                if (! empty($templateRepositoryName) && ! empty($templateOwnerName) && ! empty($templateReferenceType) && ! empty($templateReferenceValue)) {
                    ...
                    $gitCloneCommandForTemplate = $vcs->createAdapter('github')->generateCloneCommand($templateOwnerName, $templateRepositoryName, $templateReferenceValue, $templateReferenceType, $tmpTemplateDirectory, $templateRootDirectory);
                ...
                $gitCloneCommand = $adapter->generateCloneCommand($cloneOwner, $cloneRepository, $cloneVersion, $cloneType, $tmpDirectory, $rootDirectory);
                $stdout = '';
                $stderr = '';
                ...
                if (! empty($templateRepositoryName) && ! empty($templateOwnerName) && ! empty($templateReferenceType) && ! empty($templateReferenceValue)) {
                    ...
                    $exit = Console::execute('git config --global user.email ' . \escapeshellarg(APP_VCS_COMMIT_EMAIL) . ' && git config --global user.name ' . \escapeshellarg(APP_VCS_COMMIT_USERNAME) . ' && cd ' . \escapeshellarg($tmpDirectory) . ' && git checkout -b ' . \escapeshellarg($branchName) . ' && git add . && git commit -m ' . $commitMessage . ' && git push origin ' . \escapeshellarg($branchName), '', $stdout, $stderr);
                    ...
                    $deployment->setAttribute('providerCommitAuthorUrl', APP_VCS_COMMIT_AUTHOR_URL);
                    ...
                    $deployment->setAttribute('providerCommitUrl', $provider->getCommitUrl($cloneOwner, $cloneRepository, $providerCommitHash));
                    $deployment = $dbForProject->updateDocument('deployments', $deployment->getId(), new Document([
                        'providerCommitHash' => $deployment->getAttribute('providerCommitHash'),
                ...
                $this->runGitAction('processing', $adapter, $providerCommitHash, $owner, $repositoryName, $project, $resource, $deployment->getId(), $dbForProject, $dbForPlatform, $queueForRealtime, $platform);
            }
            ...
            if ($isVcsEnabled) {
                $this->runGitAction('building', $adapter, $providerCommitHash, $owner, $repositoryName, $project, $resource, $deployment->getId(), $dbForProject, $dbForPlatform, $queueForRealtime, $platform);
            }
            ...
            if ($isVcsEnabled) {
                $this->runGitAction('ready', $adapter, $providerCommitHash, $owner, $repositoryName, $project, $resource, $deployment->getId(), $dbForProject, $dbForPlatform, $queueForRealtime, $platform);
            }
            ...
            if ($isVcsEnabled) {
                $this->runGitAction('failed', $adapter, $providerCommitHash, $owner, $repositoryName, $project, $resource, $deployment->getId(), $dbForProject, $dbForPlatform, $queueForRealtime, $platform, true);
            }
    ...
    protected function runGitAction(
        ...
        try {
            if ($resource->getAttribute('providerSilentMode', false) === true || $adapter === null) {
                return;
            }
            ...
            if (! empty($providerCommitHash)) {
                ...
                $adapter->updateCommitStatus($repositoryName, $providerCommitHash, $owner, $state, $message, $providerTargetUrl, $name);
            }
            ...
            if (! empty($commentId)) {
                ...
                try {
                    ...
                    $comment->parseComment($adapter->getComment($owner, $repositoryName, $commentId));
                    $comment->addBuild($project, $resource, $resourceType, $status, $deployment->getId(), ['type' => 'logs'], $previewUrl);
                    $adapter->updateComment($owner, $repositoryName, $commentId, $comment->generateComment());
                } finally {
