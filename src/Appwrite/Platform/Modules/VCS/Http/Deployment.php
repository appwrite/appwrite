...
namespace Appwrite\Platform\Modules\VCS\Http;
...
trait Deployment
{
    protected function createGitDeployments(
        ...
        $providerKey = $adapter->getName();

        $errors = [];
        foreach ($repositories as $repository) {
            $logBase = "vcs.{$providerKey}.event.repo.unknown";
            ...
            try {
                ...
                $logBase = "vcs.{$providerKey}.event.repo.{$repositoryId}";
                    ...
                    if (!$latestComment->isEmpty()) {
                        ...
                        if ($lockAcquired) {
                            ...
                            try {
                                ...
                                $comment->parseComment($adapter->getComment($owner, $repositoryName, $latestCommentId));
                                $comment->addBuild($project, $resource, $resourceType, $commentStatus, $deploymentId, $action, $commentPreviewUrl);

                                $latestCommentId = \strval($adapter->updateComment($owner, $repositoryName, $latestCommentId, $comment->generateComment()));
                            } catch (\Throwable $e) {
                        ...
                        $comment = new Comment($platform);
                        $comment->addBuild($project, $resource, $resourceType, $commentStatus, $deploymentId, $action, $commentPreviewUrl);
                        $latestCommentId = \strval($adapter->createComment($owner, $repositoryName, $providerPullRequestId, $comment->generateComment()));
                    ...
                    if ($lockAcquired) {
                        ...
                        try {
                            ...
                            if (!empty($previewUrl)) {
                                ...
                                $adapter->updateComment($owner, $repositoryName, $latestCommentId, $comment->generateComment());
                            }
                        } catch (\Throwable $e) {
                            Console::warning("Failed to update PR comment '{$latestCommentId}': " . $e->getMessage());
                        } finally {
