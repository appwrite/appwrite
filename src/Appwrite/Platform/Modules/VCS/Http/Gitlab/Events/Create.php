...
class Create extends Action
{
    ...
    {
        try {
            return $installationTokens->adapterForRepository($repository, 'gitlab', $dbForPlatform, $vcsFactory);
        } catch (\Throwable $error) {
            $installationId = $repository->getAttribute('installationId', '');
            $message = "Failed to resolve GitLab adapter for installation '{$installationId}': " . $error->getMessage();
            Console::warning($message);
            Span::add("vcs.gitlab.event.installation.{$installationId}.error", $message);
            $errors[] = $message;
            return null;
        }
    }
