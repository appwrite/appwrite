    ...
    public function action(
        ...
        if ($installation->getAttribute('personal', false) === true) {
            ...
            if (!\method_exists($oauth2, 'createRepository')) {
                throw new Exception(Exception::GENERAL_PROVIDER_FAILURE, $provider->getName() . ' does not support personal repository creation.');
            }

            try {
                $repository = $oauth2->createRepository($accessToken, $name, $private);
            } catch (Exception $exception) {
                throw new Exception(Exception::GENERAL_PROVIDER_FAILURE, $provider->getName() . " failed to process the request: " . $exception->getMessage());
            }
        } else {
            ...
            try {
                $repository = $adapter->createRepository($owner, $name, $private);
            } catch (Exception $exception) {
                throw new Exception(Exception::GENERAL_PROVIDER_FAILURE, $provider->getName() . " failed to process the request: " . $exception->getMessage());
            }
        }
