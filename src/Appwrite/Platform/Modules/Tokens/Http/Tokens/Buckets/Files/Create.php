...
class Create extends Action
{
    ...
    // Add cache header params with validation
    ->param('maxAge', null, new Nullable(new \Appwrite\Utopia\Database\Validator\Integer(0, 31536000)), 'Max-Age value in seconds for Cache-Control header', true)
    ->param('expires', null, new Nullable(new \Appwrite\Utopia\Database\Validator\HttpDate()), 'Expires header value (RFC 1123 date)', true)
    ->param('lastModifiedMode', null, new Nullable(new \Appwrite\Utopia\Database\Validator\Enum(['static','date','file'])), 'Last-Modified mode: static, date, or file', true)
    ->param('lastModifiedStatic', null, new Nullable(new \Appwrite\Utopia\Database\Validator\HttpDate()), 'Static Last-Modified value (RFC 1123 date)', true)
    ->param('vary', null, new Nullable(new \Utopia\Validator\Text(0, 128)), 'Vary header value', true)
    ->param('pragma', null, new Nullable(new \Utopia\Validator\Text(0, 128)), 'Pragma header value', true)
        ->inject('response')
        ->inject('dbForProject')
