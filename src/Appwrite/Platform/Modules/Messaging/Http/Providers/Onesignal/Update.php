...
namespace Appwrite\Platform\Modules\Messaging\Http\Providers\Onesignal;
...
class Update extends Action
{
    ...
    {
        return 'updateOnesignalProvider';
    }
    ...
    public function __construct()
    {
        $this
            ...
            ->setHttpPath('/v1/messaging/providers/onesignal/:providerId')
            ->desc('Update OneSignal provider')
            ...
            ->label('sdk', new Method(
                namespace: 'messaging',
                group: 'providers',
                name: 'updateOnesignalProvider',
                description: '/docs/references/messaging/update-onesignal-provider.md',
            ...
            ->param('appId', '', new Text(0), 'OneSignal application ID.', true)
            ->param('restApiKey', '', new Text(0), 'OneSignal REST API key.', true)
    ...
    public function action(string $providerId, string $name, ?bool $enabled, string $appId, string $restApiKey, Event $queueForEvents, Database $dbForProject, Response $response)
    {
        ...
        if (!\is_null($enabled)) {
            if ($enabled) {
                if (
                    \array_key_exists('appId', $credentials) &&
                    \array_key_exists('restApiKey', $credentials)
                ) {
