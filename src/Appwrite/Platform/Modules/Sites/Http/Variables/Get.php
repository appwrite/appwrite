...
class Get extends Base
{
    ...
    public function __construct()
    {
        $this
            ...
            ->param('siteId', '', fn (Database $dbForProject) => new UID($dbForProject->getAdapter()->getMaxUIDLength()), 'Site unique ID.', true, ['dbForProject'])
            ->param('variableId', '', fn (Database $dbForProject) => new UID($dbForProject->getAdapter()->getMaxUIDLength()), 'Variable unique ID.', true, ['dbForProject'])
            ->inject('response')
            ->inject('dbForProject')
