...
class XList extends Base
{
    ...
    public function __construct()
    {
        $this
            ...
            ->param('status', null, new Nullable(new WhiteList([self::STATUS_PENDING, self::STATUS_STARTED, self::STATUS_ENDED, self::STATUS_UPLOADING, self::STATUS_READY, self::STATUS_ERROR, self::STATUS_ABORTED], true)), 'Only return renditions in this transcoding state.', true, enum: new Enum(name: 'VideoRenditionStatus'))
            ->inject('response')
            ->inject('dbForProject')
            ->inject('user')
