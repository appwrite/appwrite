...
class V27 extends Filter
{
    ...
    /**
     * Pre-1.9.6 SDKs send a single composite `resourceId` of the form
     * `{databaseId}:{collectionId}`. The 1.9.6 endpoints take separate
     * `databaseId` and `collectionId` UIDs, so split the composite here
     * before validation runs. Explicit `databaseId`/`collectionId` keys
     * (sent by 1.9.6+ SDKs) take precedence.
     */
