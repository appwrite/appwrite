...
class PresenceHandler extends Action
{
    public function __construct()
    {
        $this
            ...
            ->param('presenceId', 'unique()', new Text(36), 'Presence document ID', optional: true)
            ->param('metadata', null, new JSON(), 'Optional metadata payload', optional: true, skipValidation: true)
            ->param('permissions', null, new ArrayList(new Text(2048)), 'Optional permissions list', optional: true)
            ->inject('connection')
            ->inject('server')
            ->inject('realtime')
            ->inject('database')
