...
impl Http {
    ...
    /// When enabled, each request is printed via [`utopia_console::Console`]
    /// (`method`, URI, status, duration). Internal `/_health` probes are skipped.
    ...
    pub async fn run(&self, mut request: Request, response: Response) -> Result<()> {
        ...
        let verbose_method = self.verbose.then(|| request.method().to_string());
        let verbose_uri = self.verbose.then(|| request.uri().to_string());
        ...
        if let (Some(method), Some(uri)) = (verbose_method, verbose_uri) {
            log_verbose_request(&method, &uri, response.status_code(), response.elapsed());
        }
        Ok(())
    }
...
fn log_verbose_request(method: &str, uri: &str, status: u16, elapsed: std::time::Duration) {
    let path = uri.split('?').next().unwrap_or(uri);
    if path == "/_health" {
        return;
    }
    let ms = elapsed.as_secs_f64() * 1000.0;
    let line = format!("[{status}] {method} {uri}  {ms:.2}ms");
    let _ = if status >= 500 {
        Console::error(&line)
    } else if status >= 400 {
