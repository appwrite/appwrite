//! Port of `tests/e2e/DNS/Resolver/CloudflareTest.php`.
//! PHP talks UDP to 1.1.1.1; we point `Cloudflare::with_nameserver` at an in-process zone.

mod common;

use common::{dns_query, start_google_com};
use utopia_dns::message::Record;
use utopia_dns::resolver::{Cloudflare, Resolver};

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn resolve_google_a() {
    let server = start_google_com().await;
    let port = server.udp.unwrap().port();
    let resolver = Cloudflare::with_nameserver("127.0.0.1", port).unwrap();
