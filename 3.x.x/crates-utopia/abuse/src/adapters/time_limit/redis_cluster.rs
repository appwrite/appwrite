...
use crate::logs::Logs;
use crate::redis_ops::{slice_logs, ClusterConnectionExt};
use crate::time_util::{align_timestamp, unix_now};

use super::redis_key;
use super::NAMESPACE;

/// PHP `Utopia\Abuse\Adapters\TimeLimit\RedisCluster`.
...
impl RedisCluster<ClusterConnection> {
    /// PHP `new RedisCluster($key, $limit, $seconds, $redis)`.
    #[must_use]
    pub fn new(
        ...
        Self::with_connection(key, limit, seconds, connection)
    }

    /// Open a cluster client from node URLs (`redis://host:port`).
    ///
    /// # Errors
    ///
...
impl<C: ClusterConnectionExt> RedisCluster<C> {
    /// Build against any [`ClusterConnectionExt`] implementation.
    #[must_use]
    ...
    fn count(&mut self, key: &str, timestamp: i64) -> Result<i64, AbuseError> {
        ...
        let raw = self.conn.lock().get_string(&redis_key)?;
        let count = raw
            .as_deref()
    ...
    fn hit(&mut self, key: &str, timestamp: i64) -> Result<(), AbuseError> {
        ...
        let redis_key = redis_key(key, timestamp);
        self.conn.lock().incr_expire(&redis_key, self.ttl)?;
    ...
    fn set(&mut self, key: &str, timestamp: i64, value: i64) -> Result<(), AbuseError> {
        ...
        self.conn
            .lock()
            .set_expire(&redis_key, &value.to_string(), self.ttl)?;
