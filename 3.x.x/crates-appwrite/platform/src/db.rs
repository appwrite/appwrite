...
use utopia_pools::{
    BoxError, Connection, Pool, PoolError, Recover, RecoverCall, ResourceGuard, Swoole,
};
        ...
        match value.trim().to_ascii_lowercase().as_str() {
            ...
            other => {
                let _ = Console::warning(&format!(
                    "Unknown _APP_DB_ADAPTER={other:?}; falling back to memory"
                ));
                Self::Memory
            }
        }
...
impl ProjectDatabase {
    ...
    pub fn lock(&self) -> ProjectDbGuard<'_> {
        match &self.inner {
            ProjectDatabaseInner::Memory(mutex) => ProjectDbGuard::Memory(
                mutex
                    .lock()
...
impl DatabasePool {
    ...
    pub fn get_or_create(
        ...
        let db = match &self.live {
            ...
            None => {
                ProjectDatabase::memory(ProjectDb::Memory(new_memory_project_database(project_id)))
            }
        };
...
fn shared_cache() -> Cache {
    ...
    match CacheRedis::connect_url(&format!("redis://{credentials}{host}:{port}/")) {
        ...
        Err(err) => {
            let _ = Console::warning(&format!(
                "Redis cache connect failed ({err}); falling back to an in-process cache, which PHP cannot invalidate"
            ));
...
fn warm_up_pool(pool: &Pool<ProjectDb>, name: &str) -> Result<(), DatabaseError> {
    let connection = pop_sync(pool).map_err(|err| {
        DatabaseError::database(format!(
            "dbForProject pool warm-up failed for {name}: {err}"
        ))
    })?;
    connection.reclaim();
    Ok(())
