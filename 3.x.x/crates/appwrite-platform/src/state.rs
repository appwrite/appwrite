...
impl AppwriteState {
    ...
    pub fn connect_from_env() -> (Self, &'static str) {
        ...
        if !crate::db::feature_enabled(kind) {
            eprintln!(
                "appwrite-platform: _APP_DB_ADAPTER={} requested but this binary was built \
                 without that adapter feature; falling back to in-memory state",
                kind.as_str()
            );
            return (Self::default(), AdapterKind::Memory.as_str());
        }
        ...
        let Some(config) = DatabaseConfig::from_env(kind) else {
            eprintln!(
                "appwrite-platform: _APP_DB_ADAPTER={} but _APP_DB_HOST is unset; \
        ...
        match crate::db::new_platform_database_pool(&config, pool_size, pool_timeout) {
            ...
            Err(err) => {
                eprintln!(
                    "appwrite-platform: dbForPlatform {} connect failed ({err}); \
                     falling back to in-memory state",
                    kind.as_str()
                );
