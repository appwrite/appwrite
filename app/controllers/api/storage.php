...
App::get('/v1/storage/buckets/:bucketId/files/:fileId/preview')
    ...
    ->param('gravity', Image::GRAVITY_CENTER, new WhiteList(Image::getGravityTypes()), 'Image crop gravity. Can be one of ' . implode(', ', \array_map(fn ($node) => "`$node`", Image::getGravityTypes())), true)
    ...
    ->param('output', '', new WhiteList(\array_keys(Config::getParam('storage-outputs')), true), 'Output format type. Can be one of ' . \implode(', ', \array_map(fn ($node) => "`$node`", \array_keys(Config::getParam('storage-outputs')))), true)
    ->inject('request')
    ->inject('response')
    ->inject('project')
    ->inject('dbForProject')
