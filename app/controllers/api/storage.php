...
App::get('/v1/storage/buckets')
    ...
    ->action(function (array $queries, string $search, Response $response, Database $dbForProject) {
        ...
        $response->dynamic(new Document([
            ...
            'total' => $dbForProject->count('buckets', $filterQueries, (int) App::getEnv('_APP_LIMIT_COUNT', APP_LIMIT_COUNT)),
...
App::get('/v1/storage/buckets/:bucketId/files')
    ...
    ->action(function (string $bucketId, array $queries, string $search, Response $response, Database $dbForProject, string $mode) {
        ...
        if ($fileSecurity && !$valid) {
            ...
            $total = $dbForProject->count('bucket_' . $bucket->getInternalId(), $filterQueries, (int) App::getEnv('_APP_LIMIT_COUNT', APP_LIMIT_COUNT));
        } else {
            $files = Authorization::skip(fn () => $dbForProject->find('bucket_' . $bucket->getInternalId(), $queries));
            $total = Authorization::skip(fn () => $dbForProject->count('bucket_' . $bucket->getInternalId(), $filterQueries, (int) App::getEnv('_APP_LIMIT_COUNT', APP_LIMIT_COUNT)));
        }
