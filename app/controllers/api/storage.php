...
App::get('/v1/storage/buckets/:bucketId')
    ...
    ->param('bucketId', '', fn (Database $dbForProject) => new UID($dbForProject->getAdapter()->getMaxUIDLength()), 'Bucket unique ID.', true, ['dbForProject'])
    ->inject('response')
    ->inject('dbForProject')
...
App::post('/v1/storage/buckets/:bucketId/files')
    ...
    ->action(function (string $bucketId, string $fileId, mixed $file, ?array $permissions, Request $request, Response $response, Database $dbForProject, Document $user, Event $queueForEvents, string $mode, Device $deviceForFiles, Device $deviceForLocal) {
        ...
        if (!empty($contentRange)) {
            ...
            $idValidator = new UID($dbForProject->getAdapter()->getMaxUIDLength());
...
App::get('/v1/storage/buckets/:bucketId/files/:fileId')
    ...
    ->param('bucketId', '', fn (Database $dbForProject) => new UID($dbForProject->getAdapter()->getMaxUIDLength()), 'Storage bucket unique ID. You can create a new storage bucket using the Storage service [server integration](https://appwrite.io/docs/server/storage#createBucket).', true, ['dbForProject'])
    ->param('fileId', '', fn (Database $dbForProject) => new UID($dbForProject->getAdapter()->getMaxUIDLength()), 'File ID.', true, ['dbForProject'])
    ->inject('response')
    ->inject('dbForProject')
    ->inject('mode')
...
App::get('/v1/storage/buckets/:bucketId/files/:fileId/download')
    ...
    ->param('bucketId', '', fn (Database $dbForProject) => new UID($dbForProject->getAdapter()->getMaxUIDLength()), 'Storage bucket ID. You can create a new storage bucket using the Storage service [server integration](https://appwrite.io/docs/server/storage#createBucket).', true, ['dbForProject'])
    ->param('fileId', '', fn (Database $dbForProject) => new UID($dbForProject->getAdapter()->getMaxUIDLength()), 'File ID.', true, ['dbForProject'])
    // NOTE: this is only for the sdk generator and is not used in the action below and is utilised in `resources.php` for `resourceToken`.
    ->param('token', '', new Text(512), 'File token for accessing this file.', true)
    ->inject('request')
...
App::get('/v1/storage/:bucketId/usage')
    ...
    ->param('bucketId', '', fn (Database $dbForProject) => new UID($dbForProject->getAdapter()->getMaxUIDLength()), 'Bucket ID.', true, ['dbForProject'])
    ->param('range', '30d', new WhiteList(['24h', '30d', '90d'], true), 'Date range.', true)
    ->inject('response')
    ->inject('project')
    ->inject('dbForProject')
