    ...
    ->action(function (throwable $error, Request $request, Database $dbForProject, App $utopia) {
        try {
            ...
            if ($error instanceof Timeout) {
                ...
                $document = Authorization::skip(fn() => $dbForProject->getDocument('slow_queries', $key));
                if ($document->isEmpty()) {
                    $document = Authorization::skip(fn()=>$dbForProject->createDocument('slow_queries', new Document([
                    '$id' => $key,
                    'blocked' => false,
                    'count' => 1,
                    'queries' => $queries,
...
App::get('/v1/databases/:databaseId/collections/:collectionId/documents')
    ...
    ->action(function (string $databaseId, string $collectionId, array $queries, Response $response, Database $dbForProject, string $mode) {
        ...
        var_dump('_APP_SLOW_QUERIES_ACTIVE');
        var_dump(App::getEnv('_APP_SLOW_QUERIES_ACTIVE'));
        ...
        if ($documentSecurity && !$valid) {
            ...
            $documents = Authorization::skip(fn () => $dbForProject->find('database_' . $database->getInternalId() . '_collection_' . $collection->getInternalId(), $queries, $timeout));
