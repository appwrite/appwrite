...
App::get('/v1/vcs/github/installations/:installationId/providerRepositories/:providerRepositoryId/contents')
    ...
    ->param('providerRef', '', new Text(256, 0), 'Git reference (branch, tag, commit) to get contents from', true)
    ->inject('gitHub')
    ->inject('response')
    ->inject('project')
    ->inject('dbForPlatform')
    ->action(function (string $installationId, string $providerRepositoryId, string $providerRootDirectory, string $providerRef, GitHub $github, Response $response, Document $project, Database $dbForPlatform) {
        ...
        $contents = $github->listRepositoryContents($owner, $repositoryName, $providerRootDirectory, $providerRef);
