...
return function (Container $container): void {
    ...
    $container->set('user', function (Request $request, Document $project, Document $console, Authorization $authorization) use ($getMode, $getDbForPlatform, $getDbForProject) {
        ...
        $impersonateUserId = $request->getHeader('x-appwrite-impersonate-user-id', (string)$request->getQuery('impersonateUserId', ''));
        $impersonateEmail = $request->getHeader('x-appwrite-impersonate-user-email', (string)$request->getQuery('impersonateEmail', ''));
        $impersonatePhone = $request->getHeader('x-appwrite-impersonate-user-phone', (string)$request->getQuery('impersonatePhone', ''));
