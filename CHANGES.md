...
* Combined worker and scheduler containers are the default Docker Compose topology. Separate per-queue containers remain available via `--profile separate` / `--topology=separate`. `_APP_QUEUE_ADAPTER=inline` runs jobs in the publishing process so the stack can omit worker containers (`docker-compose.inline.yml`; the development override uses this). Multi-queue worker APIs live in [utopia-php/monorepo](https://github.com/utopia-php/monorepo) (`packages/queue`, `packages/platform`); see [utopia-php/monorepo#136](https://github.com/utopia-php/monorepo/pull/136) and the Inline adapter in [utopia-php/monorepo#146](https://github.com/utopia-php/monorepo/pull/146).
