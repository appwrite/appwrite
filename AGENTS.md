...
The whole dev stack runs through Docker Compose; nothing runs directly on the host. Standard build/run/test commands are in the [Commands](#commands) table above and in `CONTRIBUTING.md` (dev port table) — this section only captures the non-obvious cloud-VM caveats.

- **Start the Docker daemon first.** The VM has Docker installed but no init system, so `dockerd` is not running at session start. Start it once per session (e.g. `sudo dockerd > /tmp/dockerd.log 2>&1 &`) and, if you hit socket permission errors, `sudo chmod 666 /var/run/docker.sock`. The daemon is configured for `fuse-overlayfs` with `containerd-snapshotter` disabled (required for Docker in this Firecracker VM); do not remove that from `/etc/docker/daemon.json`.
- **Bring up the stack** with `docker compose up -d` (the auto-merged `docker-compose.override.yml` builds the local `appwrite-dev` image the first time and mounts `./app`, `./src`, `./tests` for live edits). API + Console are served by Traefik at `http://localhost:9520` (health: `http://localhost:9520/v1/health/version`). Because source is bind-mounted, most PHP edits only need `docker compose restart <service>`, not a rebuild.
- **`composer` is NOT installed in the `appwrite-dev` runtime image** (it only exists in the Dockerfile `composer` build stage). The `composer <script>` shortcuts in the Commands table are meant to run on a host that has PHP 8.5 + Composer (as CI does). Inside the running container, invoke the tools directly instead: `docker compose exec appwrite vendor/bin/pint --test --config pint.json <path>` (lint), `docker compose exec appwrite vendor/bin/phpstan analyse -c phpstan.neon <path>` (analyze), and `docker compose exec appwrite test <path>` (PHPUnit; `test` is a bin wrapper).
- **In-tree Utopia overrides vs. the optimized classmap.** `src/Utopia/{Queue,Platform,Bus}` are PSR-4 overrides that are supposed to win over the `utopia-php/*` vendor copies. The Dockerfile runs `composer install --optimize-autoloader` in a stage where `src/` is absent, so the generated classmap can hardcode the *vendor* class path and shadow an override (e.g. `Utopia\Queue\Server`). Symptom: the combined `appwrite-worker`/`appwrite-task-scheduler` containers crash-loop with `Call to undefined method ...` and the `tests/unit/Queue` override tests error with missing helper classes, even though the API, DB, and Console are healthy. If you touch these overrides, verify the class actually resolves to `src/Utopia/...` (`ReflectionClass::getFileName()`); a clean fix regenerates the autoloader after `src/` is copied.
