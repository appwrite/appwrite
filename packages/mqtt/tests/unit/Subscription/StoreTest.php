...
namespace Utopia\Mqtt\Tests\Unit\Subscription;
...
final class StoreTest extends TestCase
{
    ...
    public function testGetConnectionReturnsTheFdRecord(): void
    {
        ...
        $this->assertSame(self::PROJECT, $connection['prefix']);
    ...
    public function testReusingAnFdUnderANewIdentityPurgesTheOldSubscriptions(): void
    {
        // The fd first belongs to projectA, then is reused for projectB. The projectA entry
        // must not linger as a stale subscriber under the old project.
        $this->store->subscribe('projectA', 'user-a', 'topic', 1, 1);
        $this->store->subscribe('projectB', 'user-b', 'topic', 1, 1);

        $this->assertSame([], $this->store->getSubscribers('projectA', 'topic'), 'old identity fully purged');
        $this->assertSame([1 => 1], $this->store->getSubscribers('projectB', 'topic'));
        $this->assertSame('projectB', $this->store->getConnection(1)['prefix']);
    }

    public function testNonSystemWildcardDoesNotMatchSystemTopics(): void
    {
        // MQTT 4.7.2: a '#' or '+' filter that does not start with '$' must not receive
        // '$'-prefixed system topics.
        $this->store->subscribe(self::PROJECT, 'user', '#', 1, 1);
        $this->store->subscribe(self::PROJECT, 'user', '+', 2, 1);

        $this->assertSame([], $this->store->getSubscribers(self::PROJECT, '$SYS/broker/uptime'));
        $this->assertSame([], $this->store->getSubscribers(self::PROJECT, '$SYS'));

        // The same wildcards still match ordinary topics.
        $this->assertArrayHasKey(1, $this->store->getSubscribers(self::PROJECT, 'app/events'));
        $this->assertArrayHasKey(2, $this->store->getSubscribers(self::PROJECT, 'app'));
    }

    public function testExplicitSystemTopicFilterStillMatches(): void
    {
        // A filter that itself begins with '$' is allowed to receive system topics.
        $this->store->subscribe(self::PROJECT, 'user', '$SYS/#', 1, 1);
        $this->store->subscribe(self::PROJECT, 'user', '$SYS/broker/uptime', 2, 1);

        $this->assertArrayHasKey(1, $this->store->getSubscribers(self::PROJECT, '$SYS/broker/uptime'));
        $this->assertArrayHasKey(2, $this->store->getSubscribers(self::PROJECT, '$SYS/broker/uptime'));
    }
}
