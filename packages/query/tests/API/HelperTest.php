...
class HelperTest extends TestCase
{
    ...
    public function testMergeBothLimitAndOffset(): void
    {
        ...
        $this->assertCount(2, $result);
        $limits = array_filter($result, fn (Query $q): bool => $q->getMethod() === Method::Limit);
        $offsets = array_filter($result, fn (Query $q): bool => $q->getMethod() === Method::Offset);
    ...
    public function testGetByTypeWithNewTypes(): void
    {
        ...
        $aggTypes = array_values(array_filter(Method::cases(), fn (Method $m): bool => $m->isAggregate()));
        $aggs = Query::getByType($queries, $aggTypes);
        $this->assertCount(2, $aggs);

        $joinTypes = array_values(array_filter(Method::cases(), fn (Method $m): bool => $m->isJoin()));
