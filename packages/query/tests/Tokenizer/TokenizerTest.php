...
namespace Utopia\Query\Tests\Tokenizer;
...
class TokenizerTest extends TestCase
{
    ...
    {
        return array_map(fn (Token $t): TokenType => $t->type, $tokens);
    }
    ...
    {
        return array_map(fn (Token $t): string => $t->value, $tokens);
    }
    ...
    public function testStringLiteralWithEscapedQuote(): void
    {
        ...
        $stringToken = array_find($tokens, fn ($t): bool => $t->type === TokenType::String);
    ...
    public function testOperators(): void
    {
        ...
        $operators = array_values(array_filter(
            $tokens,
            fn (Token $t): bool => $t->type === TokenType::Operator
        ));

        $this->assertSame(
            ['=', '!=', '<>', '<', '>', '<=', '>='],
            array_map(fn (Token $t): string => $t->value, $operators)
        );
    }
    ...
    public function testMultipleJoins(): void
    {
        ...
        $keywords = array_values(array_filter(
            $tokens,
            fn (Token $t): bool => $t->type === TokenType::Keyword
        ));
        $kwValues = array_map(fn (Token $t): string => $t->value, $keywords);
    ...
    public function testLineComment(): void
    {
        ...
        $comments = array_values(array_filter(
            $all,
            fn (Token $t): bool => $t->type === TokenType::LineComment
        ));
    ...
    public function testBlockComment(): void
    {
        ...
        $comments = array_values(array_filter(
            $all,
            fn (Token $t): bool => $t->type === TokenType::BlockComment
        ));
    ...
    public function testStarToken(): void
    {
        ...
        $star = array_find($tokens, fn ($t): bool => $t->value === '*');
    ...
    public function testConcatOperator(): void
    {
        ...
        $pipes = array_values(array_filter(
            $tokens,
            fn (Token $t): bool => $t->type === TokenType::Operator && $t->value === '||'
        ));
