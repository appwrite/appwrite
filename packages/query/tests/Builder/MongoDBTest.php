...
class MongoDBTest extends TestCase
{
    ...
    public function testAfterBuildCallbackModifyingResult(): void
    {
        $result = new Builder()
            ->from('users')
            ->select(['name'])
            ->afterBuild(function (Statement $result): Statement {
    ...
    {
        return [
            ['set',          fn (Builder $b, string $f): Builder => $b->set([$f => 1])],
            ['push',         fn (Builder $b, string $f): Builder => $b->push($f, 1)],
            ['pull',         fn (Builder $b, string $f): Builder => $b->pull($f, 1)],
            ['pullAll',      fn (Builder $b, string $f): Builder => $b->pullAll($f, [1])],
            ['addToSet',     fn (Builder $b, string $f): Builder => $b->addToSet($f, 1)],
            ['increment',    fn (Builder $b, string $f): Builder => $b->increment($f, 1)],
            ['multiply',     fn (Builder $b, string $f): Builder => $b->multiply($f, 2)],
            ['rename',       fn (Builder $b, string $f): Builder => $b->rename($f, 'ok')],
            ['renameTarget', fn (Builder $b, string $f): Builder => $b->rename('ok', $f)],
            ['unsetFields',  fn (Builder $b, string $f): Builder => $b->unsetFields($f)],
            ['currentDate',  fn (Builder $b, string $f): Builder => $b->currentDate($f)],
            ['popFirst',     fn (Builder $b, string $f): Builder => $b->popFirst($f)],
            ['popLast',      fn (Builder $b, string $f): Builder => $b->popLast($f)],
            ['updateMin',    fn (Builder $b, string $f): Builder => $b->updateMin($f, 1)],
            ['updateMax',    fn (Builder $b, string $f): Builder => $b->updateMax($f, 1)],
            ['pushEach',     fn (Builder $b, string $f): Builder => $b->pushEach($f, [1])],
        ];
    }
     ...
     */
    #[DataProvider('fieldNameRejectingSetters')]
