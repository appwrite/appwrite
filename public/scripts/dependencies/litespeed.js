...
return this;};let get=function(name){let service=(undefined!==stock[name])?stock[name]:null;if(service==null){return null;}
...
xmlhttp.onload=function(){if(xmlhttp.readyState===4&&xmlhttp.status===200){resolve(xmlhttp.response);}
...
return null;}
function set(name,value,days){let date=new Date();date.setTime(date.getTime()+(days*24*60*60*1000));let expires=(days>0)?'expires='+date.toUTCString():'expires=0';document.cookie=name+"="+value+";"+expires+";path=/";return this;}
return{'get':get,'set':set}},true,false);window.ls.container.set('view',function(http,container){let stock={};let execute=function(view,node,container){container.set('element',node,true,false);container.resolve(view.controller);if(view.repeat!==true){node.removeAttribute(view.selector);}};let parse=function(node,skip,callback){if(node.tagName==='SCRIPT'){return;}
...
if(result===null||undefined===result){result=def;}
...
router.reset();if(route===null){return;}
scope.template=(undefined!==route.view.template)?route.view.template:null;scope.controller=(undefined!==route.view.controller)?route.view.controller:function(){};document.dispatchEvent(new CustomEvent('state-change'));if(firstFromServer&&router.getPrevious()===null){scope.template='';document.dispatchEvent(new CustomEvent('state-changed'));}
else if(count===1){view.render(element,function(){document.dispatchEvent(new CustomEvent('state-changed'));});}
else if(router.getPrevious()!==null){view.render(element,function(){document.dispatchEvent(new CustomEvent('state-changed'));});}};let findParent=function(tagName,el){if((el.nodeName||el.tagName).toLowerCase()===tagName.toLowerCase()){return el;}
...
if((target.hasAttribute('target'))&&(target.getAttribute('target')==='_blank')){return false;}
if(target.hostname!==window.location.hostname){return false;}
let route=router.match(target);if(route===null){return false;}
event.preventDefault();if(window.location===target.href){return false;}
route.view.state=(undefined===route.view.state)?true:route.view.state;if(route.view.state===true){if(router.getPrevious()&&router.getPrevious().view&&(router.getPrevious().view.scope!==route.view.scope)){window.location.href=target.href;return false;}
...
return;}
if(type==='checkbox'){if(typeof value==='boolean'||value==='true'||value==='false'){if(value===true||value==='true'){element.setAttribute('checked','checked');element.checked=true;}
...
let children=[];element.$lsSkip=true;element.style.visibility=(array.length===0&&element.style.visibility=='')?'hidden':'visible';for(let prop in array){if(counter==limit){break;}
